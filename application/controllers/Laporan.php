...
class Laporan extends CI_Controller {
	...
	public function exportk1($id)
	{
		$data['periode'] = $this->input->post('tahun');
		$data['jenispenerimaan'] = $this->jenis_rencana_penerimaan_k1_model->get();
		$data['kegiatan'] = $this->jenis_kegiatan_model->get();
		$data['nama'] = $this->session->nama_sekolah;
		$data['sumjenisPenerimaan'] = $this->jenis_rencana_penerimaan_k1_model->sumJenisPenerimaan();
		$data['idJenis'] = $this->jenis_rencana_penerimaan_k1_model->getIdJenisPenerimaan();
		$data['subPenerimaan'] = $this->jenis_rencana_penerimaan_k1_model->get_by_id();
		$data['number'] = 1;

		$action = $this->input->post('action');
		if($action == 'lihat') {
			$this->load->view('laporan/k1', $data);
		}
		if($action == 'unduh') {
			$this->load->view('laporan/k1', $data);
		}		
